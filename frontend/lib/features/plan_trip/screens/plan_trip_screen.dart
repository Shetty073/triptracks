...
class _PlanTripScreenState extends ConsumerState<PlanTripScreen> {
  ...
  Future<void> _addLocation(String label, ValueChanged<LocationSuggestion> onSelected) async {
  ...
  Future<void> _calculatePlan() async {
    if (_source == null || _destination == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Source and Destination are required')));
      return;
    }
    
    setState(() => _isLoading = true);
    try {
      final intel = await ref.read(planTripProvider).calculateItinerary(
        source: _source!,
  ...
  Future<void> _saveDraft() async {
    if (_titleController.text.isEmpty || _source == null || _destination == null) {
       ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Title, Source and Destination are required')));
       return;
    }
        ...
        : ListView(
            ...
            children: [
               TextField(
                ...
                decoration: const InputDecoration(labelText: 'Trip Title (e.g. Summer Road Trip)'),
              ),
              ...
              ListTile(
                ...
                onTap: () => _addLocation('Source', (loc) => setState(() => _source = loc)),
              ),
              ...
              ListTile(
                ...
                tileColor: Colors.white,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                onTap: () => _addLocation('Destination', (loc) => setState(() => _destination = loc)),
              ),
              ...
              Row(
                ...
                children: [
                  const Text('Stops (Optional)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
                  IconButton(
                    icon: const Icon(Icons.add_location_alt, color: Colors.deepPurple),
                    onPressed: () => _addLocation('Stop', (loc) => setState(() => _stops.add(loc))),
                  ),
                ],
              ),
                ...
                Card(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                         const Text('Trip Intelligence', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                         const SizedBox(height: 8),
                         Text('Total Distance: ${_itineraryData!['total_distance_km']} km'),
                         Text('Estimated Drive Time: ${(_itineraryData!['total_estimated_time_mins'] / 60).toStringAsFixed(1)} hours'),
                         Text('Estimated Days (Based on your capability): ${_itineraryData!['estimated_days']} days'),
                         if ((_itineraryData!['suggestions'] as List).isNotEmpty) ...[
                           const SizedBox(height: 16),
                           const Text('⚠️ Smart Suggestions:', style: TextStyle(color: Colors.orange, fontWeight: FontWeight.bold)),
                           ...(_itineraryData!['suggestions'] as List).map((s) => Text('- ${s['segment']}: ${s['reason']}')),
                         ],
                      ],
                    ),
                  )
...
class _LocationSearchDelegate extends SearchDelegate<LocationSuggestion?> {
  ...
  Widget _buildSuggestions(BuildContext context) {
    if (query.isEmpty) return const Center(child: Text('Type to search locations'));
